...
class InventoryApp:
    ...
    def update_catalog_excel(self):
        ...
        for product in self.inventory:
            catalog.append({
                ...
                "כמות מינימום": product.get("min_quantity", MINIMUM_STOCK),
    ...
    def get_status(self, product):
        ...
        min_quantity = product.get("min_quantity", MINIMUM_STOCK)
    ...
    def create_widgets(self):
        ...
        warning = tk.Label(
            self.root,
            text="⚠️ שים לב: מדובר במלאי רגיש. אין לאפשר חוסר בפריטים. כמות המינימום הקבועה לכל פריט היא 15. כל ירידה מתחת למינימום תסומן כהתראה.",
            font=("Arial", 12, "bold"),
            fg="red"
        )
        warning.pack(pady=5)
        ...
        self.min_quantity_entry.insert(0, str(MINIMUM_STOCK))
        self.min_quantity_entry.config(state="readonly")
    ...
    def fill_fields(self, event):
        ...
        if selected:
            ...
            self.min_quantity_entry.config(state="normal")
